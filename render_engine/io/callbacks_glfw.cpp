...
void Callbacks_GLFW::registerWindow(GLFWwindow* wnd, RenderEngine* engine) {
	RenderEngine* existing = lookup_engine(wnd);
	if (existing) {
...
void Callbacks_GLFW::_framebufferSizeCallback(
	...
	RenderEngine* engine = lookup_engine(wnd);
	if (engine) {
...
RenderEngine* Callbacks_GLFW::lookup_engine(GLFWwindow* wnd) {
	if (wnd == primary.wnd) {
